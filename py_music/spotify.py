...
from metascript import forceRemoveTags
...
def metadataGen(path, filename):
    s_query = forceRemoveTags(filename)
    ...
    while 1:
        ...
        if s_obj["image_url"] == "":
            s_query = input("Couldnt find in spotify, give a different name : ")
            if s_query == "":
                title = (input("Enter new title : ") or filename)
